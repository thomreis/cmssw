...
upgradeWFs['ecalDevelAlpakaSpikes'] = UpgradeWorkflow_ecalDevel(
    digi = {
    ...
    def setup_(self, step, stepName, stepDict, k, properties):
        ...
        if 'Digi' in step and 'NoHLT' not in step:
            ...
            if self.__ecalTPPh2 is not None:
                ...
                stepDict[stepName][k] = merge([mods, stepDict[step][k]])
        if 'RecoGlobal' in step:
            stepDict[stepName][k] = merge([{'-s': 'RAW2DIGI,RECO,RECOSIM,PAT,VALIDATION:@ecalOnlyValidation',
                                            '--datatier':'GEN-SIM-RECO,DQMIO',
                                            '--eventcontent':'FEVTDEBUGHLT,DQM',
                                        }, stepDict[step][k]])
            if self.__ecalTPPh2 is not None:
                stepDict[stepName][k] = merge([{'--era': stepDict[step][k]['--era']+',phase2_ecal_devel,phase2_ecalTP_devel'}, stepDict[step][k]])
        if 'HARVESTGlobal' in step:
            stepDict[stepName][k] = merge([{'-s': 'HARVESTING:@ecalOnlyValidation', }, stepDict[step][k]])
