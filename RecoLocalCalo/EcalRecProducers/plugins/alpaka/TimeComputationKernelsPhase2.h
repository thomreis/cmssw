#ifndef RecoLocalCalo_EcalRecProducers_plugins_alpaka_TimeComputationKernelsPhase2_h
#define RecoLocalCalo_EcalRecProducers_plugins_alpaka_TimeComputationKernelsPhase2_h
...
namespace ALPAKA_ACCELERATOR_NAMESPACE::ecal::multifit {
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE bool use_sample(unsigned int sample_mask, unsigned int sample) {
    return sample_mask & (0x1 << (EcalDataFrame_Ph2::MAXSAMPLES - (sample + 1)));
  }
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  ...
                                  ConfigurationParametersPhase2::type const timeFitLimits_firstEB,
                                  ConfigurationParametersPhase2::type const timeFitLimits_secondEB) const {
      // constants
      ...
      for (auto block : cms::alpakatools::uniform_groups(acc, totalElements)) {
        for (auto idx : cms::alpakatools::uniform_group_elements(acc, block, totalElements)) {
          ...
          auto const amplitudeFitParameters = paramsDev->amplitudeFitParamsEB.data();
          auto const timeFitParameters = paramsDev->timeFitParamsEB.data();
          auto const timeFitParameters_size = paramsDev->timeFitParamsEB.size();
          auto const timeFitLimits_first = timeFitLimits_firstEB;
          auto const timeFitLimits_second = timeFitLimits_secondEB;
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  ...
                                  EcalMultifitParametersPhase2 const* paramsDev) const {
      /// launch ctx parameters are
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
      ...
      for (auto gtx : cms::alpakatools::uniform_elements(acc, nchannels * nsamples)) {
        auto const elemIdx = gtx % elemsPerBlock;
        auto const sample = elemIdx % nsamples;
        ...
        auto const* digis = digisDevEB.data().data()->data();

        auto const gainIdPrev = ecalMGPA::gainId(digis[gtx - 1]);
        auto const gainIdNext = ecalMGPA::gainId(digis[gtx]);
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
      ...
      for (auto txforward : cms::alpakatools::uniform_elements(acc, nchannels * nsamples)) {
        ...
        // indices/inits
        auto const sample = tx % nsamples;
        auto const input_ch_start = ch * nsamples;
        ...
        auto const sample_mask = conditionsDev.sampleMask_EB();
        ...
        auto const gainId = ecalMGPA::gainId(digis[tx]);
        auto const adc = ecalMGPA::adc(digis[tx]);
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  ...
                                  ConfigurationParameters::type const outOfTimeThreshG61mEB) const {
      // constants
      constexpr auto nsamples = EcalDataFrame_Ph2::MAXSAMPLES;
      auto const nchannels = digisDevEB.size();
      ...
      for (auto gtx : cms::alpakatools::uniform_elements(acc, nchannels)) {
        ...
        auto g_amplitude = uncalibRecHitsEB.amplitude();
        auto g_jitter = uncalibRecHitsEB.jitter();
        auto g_jitterError = uncalibRecHitsEB.jitterError();
        ...
        auto const amplitudeBins = conditionsDev.timeBiasCorrections_amplitude_EB().data();
        auto const shiftBins = conditionsDev.timeBiasCorrections_shift_EB().data();
        auto const amplitudeBinsSize = conditionsDev.timeBiasCorrectionSizeEB();
        auto const timeConstantTerm = timeConstantTermEB;
        auto const timeNconst = timeNconstEB;
        auto const offsetTimeValue = conditionsDev.timeOffset_EB();
        auto const amplitudeThreshold = amplitudeThresholdEB;
        auto const outOfTimeThreshG12p = outOfTimeThreshG12pEB;
        auto const outOfTimeThreshG12m = outOfTimeThreshG12mEB;
        auto const outOfTimeThreshG61p = outOfTimeThreshG61pEB;
        auto const outOfTimeThreshG61m = outOfTimeThreshG61mEB;
        ...
        if (amplitude > amplitudeThreshold * rms_x12) {
          ...
          if (amplitude > 3000.) {
            for (int isample = 0; isample < nsamples; isample++) {
              auto const gainid = ecalMGPA::gainId(digis[nsamples * gtx + isample]);
              if (gainid != 1) {
...
#endif  // RecoLocalCalo_EcalRecProducers_plugins_TimeComputationKernelsPhase2_h
