        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc,
          ...
          for (auto block : cms::alpakatools::blocks_with_stride(acc, totalElements)) {
            ...
            for (auto idx : cms::alpakatools::elements_in_block(acc, block, totalElements)) {
              ...
              //
              if (sample < 5) {
                shr_hasSwitchToGain6[idx.local] =
                    shr_hasSwitchToGain6[idx.local] || shr_hasSwitchToGain6[idx.local + 5];
                shr_hasSwitchToGain1[idx.local] =
                    shr_hasSwitchToGain1[idx.local] || shr_hasSwitchToGain1[idx.local + 5];
            ...
            for (auto idx : cms::alpakatools::elements_in_block(acc, block, totalElements)) {
              ...
              if (sample < 2) {
                ...
                shr_hasSwitchToGain6[idx.local] = shr_hasSwitchToGain6[idx.local] ||
                                                  shr_hasSwitchToGain6[idx.local + 2] ||
                                                  shr_hasSwitchToGain6[idx.local + 3];
                shr_hasSwitchToGain1[idx.local] = shr_hasSwitchToGain1[idx.local] ||
                                                  shr_hasSwitchToGain1[idx.local + 2] ||
            ...
            for (auto idx : cms::alpakatools::elements_in_block(acc, block, totalElements)) {
              ...
              if (sample == 0) {
                shr_hasSwitchToGain6[idx.local] =
                    shr_hasSwitchToGain6[idx.local] || shr_hasSwitchToGain6[idx.local + 1];
                shr_hasSwitchToGain1[idx.local] =
                    shr_hasSwitchToGain1[idx.local] || shr_hasSwitchToGain1[idx.local + 1];
                shr_hasSwitchToGain0_tmp[idx.local] =
      ...
      public:
        ...
        ALPAKA_FN_ACC void operator()(TAcc const& acc,
          ...
          for (auto ndindex : cms::alpakatools::elements_with_stride_nd(acc, size_2d)) {
            ...
            auto const* G6SamplesCorrelation = isBarrel ? conditionsDev.sampleCorrelation_EB_G6().data()
                                                        : conditionsDev.sampleCorrelation_EE_G6().data();
            auto const* G1SamplesCorrelation = isBarrel ? conditionsDev.sampleCorrelation_EB_G1().data()
                                                        : conditionsDev.sampleCorrelation_EE_G1().data();
