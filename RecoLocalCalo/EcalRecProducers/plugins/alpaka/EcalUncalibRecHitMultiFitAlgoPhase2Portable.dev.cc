...
#include "TimeComputationKernelsPhase2.h"
...
namespace ALPAKA_ACCELERATOR_NAMESPACE::ecal::multifit {
  ...
  void launchKernels(Queue& queue,
    ...
    //
    Vec2D const blocks_2d{1u, nChannels};  // {y, x} coordiantes
    ...
    if (configParams.shouldRunTimingComputation) {
      ...
      auto const blocksFixMGPA = cms::alpakatools::divide_up_by(kMaxSamples * nChannels, threadsFixMGPA);
      ...
      auto const blocks_makeratio = cms::alpakatools::divide_up_by(nthreads_per_channel * nChannels, threads_makeratio);
      ...
      auto const blocks_timecorr = cms::alpakatools::divide_up_by(nChannels, threads_timecorr);
