...
std::vector<int> L1TkMuCorrDynamicWindows::make_unique_coll(const unsigned int & l1musSize, const L1TTTrackCollectionType& l1trks, std::vector<int> matches)
{
    ...
    std::vector<std::vector<int>> macthed_to_emtf(l1musSize, std::vector<int>(0)); // one vector of matched trk idx per EMTF
