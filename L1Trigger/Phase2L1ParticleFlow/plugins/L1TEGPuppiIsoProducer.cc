...
L1TEGPuppiIsoProducer<T>::L1TEGPuppiIsoProducer(const edm::ParameterSet &iConfig)
  ...
  algo_ = std::make_unique<l1ct::L1EGPuppiIsoAlgo>(
      l1ct::L1EGPuppiIsoAlgoConfig(iConfig.getParameter<std::string>("pfIsoType"),
...
template <typename T>
void L1TEGPuppiIsoProducer<T>::prepareAlgoPuppiInput(const l1t::PFCandidateCollection &l1PFCands,
                                                     l1ct::PuppiObjs &puppiObjs) {
...
template <typename T>
void L1TEGPuppiIsoProducer<T>::addPuppiIsoToOutput(const l1ct::EGIsoObjsEmu &outEgIsoObjsEmu,
                                                   std::vector<T> &outL1EGs) {
...
void L1TEGPuppiIsoProducer<T>::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  desc.add<edm::InputTag>("l1TkEmCollection", edm::InputTag("l1TkEmCollection"))
      ->setComment("TkEm or TkElectron collection");
  ...
  desc.add<double>("pfPtMin", 0.)->setComment("Minimum pT for PF candidates to be considered in isolation");
  desc.add<double>("dZMax", 0.6)
