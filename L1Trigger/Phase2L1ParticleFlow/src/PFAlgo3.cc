...
void PFAlgo3::link_tk2mu(Region &r, std::vector<int> & tk2mu, std::vector<int> & mu2tk) const {
    ...
    for (int imu = 0, nmu = r.muon.size(); imu < nmu; ++imu) {
        ...
        if (debug_) printf("PFAlgo3 \t muon  %3d (pt %7.2f, eta %+5.2f, phi %+5.2f) \n", imu, mu.floatPt(), mu.floatEta(), mu.floatPhi() );
        ...
        for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
            ...
            if (debug_ && dr < 0.4) {
                printf("PFAlgo3 \t\t possible match with track %3d (pt %7.2f, caloeta %+5.2f, calophi %+5.2f, dr %.2f, eta %+5.2f, phi %+5.2f, dr %.2f):  angular %1d, distance %.3f (vs %.3f)\n",
        ...
        if (debug_ && imatch >  -1) printf("PFAlgo3 \t muon  %3d (pt %7.2f) linked to track %3d (pt %7.2f)\n", imu, mu.floatPt(), imatch, r.track[imatch].floatPt() );
        if (debug_ && imatch == -1) printf("PFAlgo3 \t muon  %3d (pt %7.2f) not linked to any track\n", imu, mu.floatPt() );
        mu2tk[imu] = imatch;
        if (imatch > -1) {
            tk2mu[imatch] = imu;
...
void PFAlgo3::link_tk2em(Region &r, std::vector<int> & tk2em) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        if (debug_ && tk2em[itk] != -1) printf("PFAlgo3 \t track %3d (pt %7.2f) matches to EM   %3d (pt %7.2f) with dr %.3f\n", itk, tk.floatPt(), tk2em[itk], tk2em[itk] == -1 ? 0.0 : r.emcalo[tk2em[itk]].floatPt(), drbest );
    }
}
    ...
    for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
        ...
        if (ptdiff > -ptMatchLow_*pterr) {
            ...
            if (ptdiff > +ptMatchHigh_*pterr) {
                ...
                p.setFloatPt(ptdiff);
                if (debug_) printf("PFAlgo3 \t EM    %3d (pt %7.2f)    ---> promoted to electron(s) + photon (pt %7.2f)\n", iem, em.floatPt(), ptdiff);
            } else {
                em.hwFlags = 1; // may use calo momentum
                if (debug_) printf("PFAlgo3 \t EM    %3d (pt %7.2f)    ---> promoted to electron(s)\n", iem, em.floatPt());
            }
        } else {
            em.isEM = false;
            em.used = false;
...
void PFAlgo3::emtk_algo(Region & r, const std::vector<int> & tk2em, const std::vector<int> & em2ntk, const std::vector<float> & em2sumtkpterr) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        if (em.isEM) {
            ...
            if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f) matched to EM   %3d (pt %7.2f) promoted to electron with pt %7.2f\n", itk, tk.floatPt(), tk2em[itk], em.floatPt(), p.floatPt() );
        ...
        for (int iem = 0, nem = r.emcalo.size(); iem < nem; ++iem) {
            if (em2calo[iem] == ic) {
                ...
                if (em.isEM) {
                    ...
                    keepme = true;
                    if (debug_) printf("PFAlgo3 \t EM    %3d (pt %7.2f) not subtracted from calo %3d (pt %7.2f), and calo marked to be kept after EM subtraction\n", iem, em.floatPt(), ic, calo.floatPt());
                }
            }
        }
        if (pt < pt0) {
            if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f +- %7.2f) has a subtracted pt of %7.2f, empt %7.2f -> %7.2f, isem %d\n", ic, calo.floatPt(), calo.floatPtErr(), pt, ept0, ept, calo.isEM);
            calo.setFloatPt(pt);
            ...
            if (!keepme && ((emCaloUseAlsoCaloSigma_ ? pt < calo.floatPtErr() : false) || pt <= 0.125*pt0 || (calo.isEM && ept <= 0.125*ept0))) { // the <= is important since in firmware the pt0/8 can be zero
                if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f)    ----> discarded\n", ic, calo.floatPt());
                calo.used = true;
...
void PFAlgo3::link_tk2calo(Region & r, std::vector<int> & tk2calo) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
            ...
            switch (tkCaloLinkMetric_) {
                ...
                case BestByDRPt:
                    ...
                    //if (debug_ && dr < 0.2) printf("PFAlgo3 \t\t\t track %3d (pt %7.2f) vs calo %3d (pt %7.2f): dr %.3f, dq %.3f\n", itk, tk.floatPt(), ic, calo.floatPt(), dr, dq);
        ...
        if (debug_ && tk2calo[itk] != -1) printf("PFAlgo3 \t track %3d (pt %7.2f) matches to calo %3d (pt %7.2f) with dist %.3f\n", itk, tk.floatPt(), tk2calo[itk], tk2calo[itk] == -1 ? 0.0 : r.calo[tk2calo[itk]].floatPt(), drbest );
        ...
        if (debug_ && tk2calo[itk] == -1) {
            ...
            if (ibest != -1) printf("PFAlgo3 \t track %3d (pt %7.2f) would match to calo %3d (pt %7.2f) with dr %.3f if the pt min and dr requirement had been relaxed\n", itk, tk.floatPt(), ibest, r.calo[ibest].floatPt(), drbest );
        }
    }
}
...
void PFAlgo3::unlinkedtk_algo(Region & r, const std::vector<int> & tk2calo) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        if (tk.floatPt() < maxPt) {
            ...
            if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f) not matched to calo, dropped\n", itk, tk.floatPt());
...
void PFAlgo3::calo_relink(Region & r, const std::vector<int> & calo2ntk, const std::vector<float> & calo2sumtkpt, const std::vector<float> & calo2sumtkpterr) const {
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        if (addtopt[ic]) {
            auto & calo = r.calo[ic];
            if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f, sum tk pt %7.2f) is increased to pt %7.2f after merging\n", ic, calo.floatPt(), calo2sumtkpt[ic], calo.floatPt() + addtopt[ic]);
...
void PFAlgo3::linkedcalo_algo(Region & r, const std::vector<int> & calo2ntk, const std::vector<float> & calo2sumtkpt, const std::vector<float> & calo2sumtkpterr, std::vector<float> & calo2alpha) const {
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        ...
        if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f +- %7.2f, empt %7.2f) has %2d tracks (sumpt %7.2f, sumpterr %7.2f), ptdif %7.2f +- %7.2f\n", ic, calo.floatPt(), calo.floatPtErr(), calo.floatEmPt(), calo2ntk[ic], calo2sumtkpt[ic], calo2sumtkpterr[ic], ptdiff, pterr);
        if (ptdiff > +ptMatchHigh_*pterr) {
            if (ecalPriority_) {
                if (calo.floatEmPt() > 1) {
                    float emptdiff = std::min(ptdiff, calo.floatEmPt());
                    if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make photon with pt %7.2f, reduce ptdiff to %7.2f +- %7.2f\n", ic, calo.floatPt(), calo.floatEmPt(), emptdiff, ptdiff-emptdiff, pterr);
                ...
                if (ptdiff > 2) {
                    if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make also neutral hadron with pt %7.2f\n", ic, calo.floatPt(), calo.floatEmPt(), ptdiff);
                    auto & p = addCaloToPF(r, calo);
                    p.setFloatPt(ptdiff);
                    p.hwId = l1t::PFCandidate::NeutralHadron;
                }
            } else {
                if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f)    ---> promoted to neutral with pt %7.2f\n", ic, calo.floatPt(), ptdiff);
                auto & p = addCaloToPF(r, calo);
                p.setFloatPt(ptdiff);
                calo.hwFlags = 0;
            }
            ...
            calo.hwFlags = 1;
            if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f)    ---> to be deleted, will use tracks instead\n", ic, calo.floatPt());
            ...
            calo.hwFlags = 2;
            if (debug_ &&  rescaleTracks_) printf("PFAlgo3 \t calo  %3d (pt %7.2f)    ---> tracks overshoot and will be scaled down by %.4f\n", ic, calo.floatPt(), calo2alpha[ic]);
            if (debug_ && !rescaleTracks_) printf("PFAlgo3 \t calo  %3d (pt %7.2f)    ---> tracks overshoot by %.4f\n", ic, calo.floatPt(), calo2sumtkpt[ic]/calo.floatPt());
        }
        calo.used = true;
    }
}
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        if (calo.hwFlags == 1) {
            ...
            if (calo2ntk[tk2calo[itk]] == 1 && caloTrkWeightedAverage_) { 
                ...
                p.setFloatPt(ptavg);
                if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f +- %7.2f) combined with calo %3d (pt %7.2f +- %7.2f (from tk) yielding candidate of pt %7.2f\n", 
                                    itk, tk.floatPt(), tk.floatPtErr(), tk2calo[itk], calo.floatPt(), tk.floatCaloPtErr(), ptavg );
            } else {
                p.hwStatus = GoodTK_Calo_TkPt;
                if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f) linked to calo %3d promoted to charged hadron\n", itk, tk.floatPt(), tk2calo[itk]);
            }
        } else if (calo.hwFlags == 2) {
            // must rescale
            p.setFloatPt(tk.floatPt() * calo2alpha[tk2calo[itk]]); 
            p.hwStatus = GoodTk_Calo_CaloPt;
            if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f, stubs %2d chi2 %7.1f) linked to calo %3d promoted to charged hadron with pt %7.2f after maybe rescaling\n", itk, tk.floatPt(), int(tk.hwStubs), tk.hwChi2*0.1f, tk2calo[itk], p.floatPt());
        }
    }
}
...
void PFAlgo3::unlinkedcalo_algo(Region & r) const {
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        if (!r.calo[ic].used) {
            ...
            if (debug_) printf("PFAlgo3 \t calo  %3d (pt %7.2f) not linked, promoted to neutral\n", ic, r.calo[ic].floatPt());
        }
    }
}
...
void PFAlgo3::save_muons(Region &r, const std::vector<int> & tk2mu) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        if (r.track[itk].muonLink) {
            ...
            if (debug_) printf("PFAlgo3 \t track %3d (pt %7.2f) promoted to muon.\n", itk, r.track[itk].floatPt());
        }
    }
}
