...
{
    if (puppiPriors_.size() != puppiEtaCuts_.size()) throw cms::Exception("Configuration", "Mismatched lenght for puppiPriors\n");
...
void LinearizedPuppiAlgo::computePuppiWeights(Region &r, float npu, const std::vector<float> & alphaC, const std::vector<float> & alphaF) const {
    if (debug_ && npu > 0) printf("LinPup\t npu estimate %7.2f --> log(npu/200) = %+6.2f \n", npu, std::log(npu/200.f));
    for (unsigned int ip = 0, np = r.pf.size(); ip < np; ++ip) {
        ...
        if (p.hwId == l1t::PFCandidate::ChargedHadron || p.hwId == l1t::PFCandidate::Electron || p.hwId == l1t::PFCandidate::Muon) { 
            ...
            if (debug_ == 2) 
                printf("LinPup\t charged id %1d pt %7.2f eta %+5.2f phi %+5.2f   fromPV %1d                                                               --> puppi weight %.3f   puppi pt %7.2f \n", 
                        p.hwId, p.floatPt(), p.floatEta(), p.floatPhi(), p.chargedPV, p.floatPuppiW(), p.floatPt()*p.floatPuppiW());
            continue;
        }
        // neutral
        ...
        if (debug_ == 1 || debug_ == 2 || debug_ == int(10+ietaBin)) 
            printf("LinPup\t neutral id %1d pt %7.2f eta %+5.2f phi %+5.2f   alpha %+6.2f   x2a %+5.2f   x2pt %+6.2f   x2prior %+6.2f -->  x2 %+6.2f --> puppi weight %.3f   puppi pt %7.2f \n", 
                        p.hwId, p.floatPt(), p.floatEta(), p.floatPhi(), alpha, x2a, x2pt, -x2prior, x2, p.floatPuppiW(), p.floatPt()*p.floatPuppiW());
    }
}
