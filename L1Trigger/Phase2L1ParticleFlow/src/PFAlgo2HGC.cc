...
void PFAlgo2HGC::link_tk2mu(Region &r, std::vector<int> & tk2mu, std::vector<int> & mu2tk) const {
    ...
    for (int imu = 0, nmu = r.muon.size(); imu < nmu; ++imu) {
        ...
        if (debug_) printf("PFAlgo2HGC \t muon  %3d (pt %7.2f, eta %+5.2f, phi %+5.2f) \n", imu, mu.floatPt(), mu.floatEta(), mu.floatPhi() );
        ...
        for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
            ...
            if (debug_ && dr < 0.4) {
                printf("PFAlgo2HGC \t\t possible match with track %3d (pt %7.2f, caloeta %+5.2f, calophi %+5.2f, dr %.2f, eta %+5.2f, phi %+5.2f, dr %.2f):  angular %1d, distance %.3f (vs %.3f)\n",
        ...
        if (debug_ && imatch >  -1) printf("PFAlgo2HGC \t muon  %3d (pt %7.2f) linked to track %3d (pt %7.2f)\n", imu, mu.floatPt(), imatch, r.track[imatch].floatPt() );
        if (debug_ && imatch == -1) printf("PFAlgo2HGC \t muon  %3d (pt %7.2f) not linked to any track\n", imu, mu.floatPt() );
        mu2tk[imu] = imatch;
        if (imatch > -1) {
            tk2mu[imatch] = imu;
...
void PFAlgo2HGC::link_tk2calo(Region & r, std::vector<int> & tk2calo) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
            ...
            switch (tkCaloLinkMetric_) {
                ...
                case BestByDRPt:
                    ...
                    if (debug_ > 2 && dr < 0.3) printf("PFAlgo2HGC \t\t\t track %3d (pt %7.2f) vs calo %3d (pt %7.2f): dr %.3f, dq %.3f\n", itk, tk.floatPt(), ic, calo.floatPt(), dr, dq);
        ...
        if (debug_ && tk2calo[itk] != -1) printf("PFAlgo2HGC \t track %3d (pt %7.2f) matches to calo %3d (pt %7.2f) with dist %.3f (dr %.3f)\n", itk, tk.floatPt(), tk2calo[itk], r.calo[tk2calo[itk]].floatPt(), drbest, floatDR(tk,r.calo[tk2calo[itk]]) );
        ...
        if (debug_ && tk2calo[itk] == -1) {
            ...
            if (ibest != -1) printf("PFAlgo2HGC \t track %3d (pt %7.2f) would match to calo %3d (pt %7.2f) with dr %.3f if the pt min and dr requirement had been relaxed\n", itk, tk.floatPt(), ibest, r.calo[ibest].floatPt(), drbest );
        }
    }
}

void PFAlgo2HGC::sum_tk2calo(Region & r, const std::vector<int> & tk2calo, 
...
void PFAlgo2HGC::unlinkedtk_algo(Region & r, const std::vector<int> & tk2calo) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        ...
        if (tk.floatPt() < maxPt) {
            ...
            if (debug_) printf("PFAlgo2HGC \t track %3d (pt %7.2f) not matched to calo, dropped\n", itk, tk.floatPt());
            //discardTrack(r, tk, BadTK_NoCalo); // log this as discarded, for debugging
        }
    }
}

void PFAlgo2HGC::calo_relink(Region & r, const std::vector<int> & calo2ntk, const std::vector<float> & calo2sumtkpt, const std::vector<float> & calo2sumtkpterr) const {
    /// OPTIONAL STEP: try to recover split hadron showers (v1.0): 
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        ...
        for (int ic2 = 0; ic2 < nc; ++ic2) {
            ...
            //if (debug_ && dr < 0.5) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f) with no tracks is at dr %.3f from calo %3d with pt %7.2f (sum tk pt %7.2f), track excess %7.2f +- %7.2f\n", ic, calo.floatPt(), dr, ic2, calo2.floatPt(), calo2sumtkpt[ic2], calo2sumtkpt[ic2] - calo2.floatPt(), useTrackCaloSigma_ ? calo2sumtkpterr[ic2] : calo2.floatPtErr());
            if (dr < drbest) {
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        if (addtopt[ic]) {
            auto & calo = r.calo[ic];
            if (debug_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f, sum tk pt %7.2f) is increased to pt %7.2f after merging\n", ic, calo.floatPt(), calo2sumtkpt[ic], calo.floatPt() + addtopt[ic]);
...
void PFAlgo2HGC::linkedcalo_algo(Region & r, const std::vector<int> & calo2ntk, const std::vector<float> & calo2sumtkpt, const std::vector<float> & calo2sumtkpterr, std::vector<float> & calo2alpha) const {
    /// ------------- next step (needs the previous) ----------------
    // process matched calo clusters, compare energy to sum track pt
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        auto & calo = r.calo[ic];
        ...
        if (ptdiff > +ptMatchHigh_*pterr) {
            if (ecalPriority_) {
                ...
                if (ptdiff > 2) {
                    if (debug_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f, empt %7.2f)    ---> make also neutral hadron with pt %7.2f\n", ic, calo.floatPt(), calo.floatEmPt(), ptdiff);
                    auto & p = addCaloToPF(r, calo);
                    p.setFloatPt(ptdiff);
                    p.hwId = l1t::PFCandidate::NeutralHadron;
                }
            } else {
                if (debug_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> promoted to neutral with pt %7.2f\n", ic, calo.floatPt(), ptdiff);
                auto & p = addCaloToPF(r, calo);
                p.setFloatPt(ptdiff);
                calo.hwFlags = 0;
            }
            ...
            calo.hwFlags = 1;
            if (debug_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> to be deleted, will use tracks instead\n", ic, calo.floatPt());
            ...
            calo.hwFlags = 2;
            if (debug_ &&  rescaleTracks_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> tracks overshoot and will be scaled down by %.4f\n", ic, calo.floatPt(), calo2alpha[ic]);
            if (debug_ && !rescaleTracks_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f)    ---> tracks overshoot by %.4f\n", ic, calo.floatPt(), calo2sumtkpt[ic]/calo.floatPt());
        }
        calo.used = true;
    }
}

void PFAlgo2HGC::linkedtk_algo(Region & r, const std::vector<int> & tk2calo, const std::vector<int> & calo2ntk, const std::vector<float> & calo2alpha) const {
    // process matched tracks, if necessary rescale or average
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        auto & tk = r.track[itk]; 
        if (tk2calo[itk] == -1 || tk.muonLink || tk.used) continue;
        ...
        if (calo.hwFlags == 1) {
            ...
            if (calo2ntk[tk2calo[itk]] == 1 && caloTrkWeightedAverage_) { 
                ...
                p.setFloatPt(ptavg);
                if (debug_) printf("PFAlgo2HGC \t track %3d (pt %7.2f +- %7.2f) combined with calo %3d (pt %7.2f +- %7.2f (from tk) yielding candidate of pt %7.2f\n", 
...
void PFAlgo2HGC::unlinkedcalo_algo(Region & r) const {
    ...
    for (int ic = 0, nc = r.calo.size(); ic < nc; ++ic) {
        if (!r.calo[ic].used) {
            ...
            if (debug_) printf("PFAlgo2HGC \t calo  %3d (pt %7.2f) not linked, promoted to neutral\n", ic, r.calo[ic].floatPt());
        }
    }
}
...
void PFAlgo2HGC::save_muons(Region &r, const std::vector<int> & tk2mu) const {
    ...
    for (int itk = 0, ntk = r.track.size(); itk < ntk; ++itk) {
        if (r.track[itk].muonLink) {
            ...
            if (debug_) printf("PFAlgo2HGC \t track %3d (pt %7.2f) promoted to muon.\n", itk, r.track[itk].floatPt());
        }
    }
}
