...
void RegionMapper::addTrack( const l1t::PFTrack & t ) {
   ...
   for (Region &r : regions_) {
        ...
        if (inside) {
            ...
            prop.fillPropagated(t.pt(), t.trkPtError(), t.caloPtError(), r.localEta(t.caloEta()), r.localPhi(t.caloPhi()), t.quality(), t.isMuon());
            ...
            if (p.floatPt() > ptMin) {
                ...
                if (p.cluster.src) {
                    ...
                    if (match == clusterRefMap_.end()) {
                        throw cms::Exception("CorruptData") << "Invalid cluster pointer in PF candidate id " << p.hwId << " pt " << p4.pt() << " eta " << p4.eta() << " phi " << p4.phi();
                    }
                ...
                if (p.track.src) {
                    ...
                    if (match == trackRefMap_.end()) {
                        throw cms::Exception("CorruptData") << "Invalid track pointer in PF candidate id " << p.hwId << " pt " << p4.pt() << " eta " << p4.eta() << " phi " << p4.phi();
                    }
                ...
                if (p.muonsrc) {
                    ...
                    if (match == muonRefMap_.end()) {
                        throw cms::Exception("CorruptData") << "Invalid muon pointer in PF candidate id " << p.hwId << " pt " << p4.pt() << " eta " << p4.eta() << " phi " << p4.phi();
                    }
