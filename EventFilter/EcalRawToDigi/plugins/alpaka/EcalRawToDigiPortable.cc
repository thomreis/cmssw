...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  private:
    ...
    device::ESGetToken<EcalSrMappingDevice, EcalSrMappingRcd> const srMappingToken_;
  ...
  void EcalRawToDigiPortable::produce(device::Event& event, device::EventSetup const& setup) {
    ...
    EcalTriggerPrimitiveDigiDeviceCollection ecalTriggerPrimitivesDev{
        EcalTrigTowerDetId::kEBTotalTowers + 2 * EcalTrigTowerDetId::kEETowersInEta * EcalTrigTowerDetId::kEETowersInPhiPerEndcap, queue};
    ...
    if (fedCounter > 0) {
      ...
      if (unpackSrFlags_) {
        auto const& srMappingProduct = setup.getData(srMappingToken_);
        ecal::raw::unpackSrFlags(queue, inputDevice, srFlagsDevEB, srFlagsDevEE, srMappingProduct, fedCounter);
      }
      if (unpackHeader_) {
